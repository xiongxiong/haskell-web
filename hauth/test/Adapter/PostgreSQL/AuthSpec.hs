...
spec = beforeAll initDB $ do
    describe "addAuth" $
        it "should return email taken if the email already exists" $ do
            auth <- randomAuth
            runTestApp (addAuth auth >> addAuth auth) `shouldReturn` Left D.RegistrationErrEmailTaken
    
    describe "setEmailAsVerified" $
        it "should return invalid code if the code is invalid" $
            runTestApp (setEmailAsVerified "invalidCode") `shouldReturn` Left D.EmailVerificationErrInvalidCode

    describe "findUserByAuth" $ do
        it "should return Nothing if matching auth not found" $ do
            auth <- randomAuth
            runTestApp (findUserByAuth auth) `shouldReturn` Nothing
    ...
    describe "findEmailFromUserId" $ do
        it "should return Nothing if user id is not found" $
            runTestApp (findEmailFromUserId 0) `shouldReturn` Nothing
...
initDB :: IO ()
initDB = do
    conn <- connectPostgreSQL "postgresql://hauth:hauth@localhost"
...
testConf = Config
    {
          configUrl = "postgresql://hauth:hauth@localhost/hauth_test_auth"
        , configStripeCount = 2
...
randomAuth = do
    ...
    return $ either (error . show) id $ D.Auth <$> D.mkEmail email <*> D.mkPassword "abcDEF123"