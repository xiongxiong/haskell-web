...
spec = beforeAll initDB $
    describe "findUserIdBySessionId" $ do
        it "should return Nothing if session is invalid" $
            runTestApp (findUserIdBySessionId "invalidSession") `shouldReturn` Nothing

        it "should return valid user id if session is valid" $ do
            let uId = 1
            runTestApp (newSession uId >>= findUserIdBySessionId) `shouldReturn` Just uId

initDB :: IO ()
initDB = do
    let connInfo = either (error "Invalid Redis conn URL") id $ R.parseConnectInfo testConf
...
runTestApp action = withState testConf $ runReaderT action