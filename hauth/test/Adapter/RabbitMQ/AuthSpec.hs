module Adapter.RabbitMQ.AuthSpec where
...
newtype App a = App
    ...
    } deriving (Applicative, Functor, Monad, MonadIO, MonadReader (Fixture IO), Katip, KatipContext, MonadThrow, MonadCatch)
...
type PublisherStateApp a = ReaderT MQ.State IO a
...
spec = beforeAll initMQ $
    it "send and consume email verification notification should work" $
        MQ.withState testConf $ \mqState -> do
            ...
            let fixture = Fixture
                    {
                        _sendEmailVerification = \mail code -> liftIO $ putMVar mvar (mail, code)
                    }
...
initMQ :: IO ()
initMQ = do
    void $ readProcessWithExitCode "rabbitmqctl" ["delete_vhost", "hauth_test_auth"] ""
    void $ readProcessWithExitCode "rabbitmqctl" ["add_vhost", "hauth_test_auth"] ""
    void $ readProcessWithExitCode "rabbitmqctl" ["set_permissions", "-p", "hauth_test_auth", "guest", ".*", ".*", ".*"] ""

testConf :: MQ.Config
testConf = MQ.Config
    {
        MQ.configUrl = "amqp://guest:guest@localhost:5672/hauth_test_auth"
