...
spec = do
    ...
    describe "register" $ do
        it "should notify email verification upon successful registration" $ do
            ...
            let fixture = emptyFixture
                    {
                        _addAuth = \_ -> return $ Right (1, "vcode")
                        , _notifyEmailVerification = \email vCode -> atomically . writeTVar tvar $ Just (email, vCode)
                    }
            runApp fixture (register auth) `shouldReturn` Right ()
            readTVarIO tvar `shouldReturn` Just (authEmail auth, "vcode")

        it "should return failure and not notify any email verification" $ do
            let fixture = emptyFixture
                    {
                        _addAuth = \_ -> return $ Left RegistrationErrEmailTaken
                    }
            runApp fixture (register auth) `shouldReturn` Left RegistrationErrEmailTaken
    ...
    describe "verifyEmail" $ do
        it "should call the correct repo function" $ do
            ...
            runApp fixture (verifyEmail "vCode") `shouldReturn` Right ()

        it "should return the failure if writing to repo fail" $ do
            let fixture = emptyFixture
                    {
                        _setEmailAsVerified = \_ -> return $ Left EmailVerificationErrInvalidCode
                    }
            runApp fixture (verifyEmail "vCode") `shouldReturn` Left EmailVerificationErrInvalidCode

    describe "login" $ do
        it "should fail if the auth is incorrect" $ do
            let fixture = emptyFixture
                    {
                        _findUserByAuth = \_ -> return Nothing
                    }
            runApp fixture (login auth) `shouldReturn` Left LoginErrInvalidAuth

        it "should fail if the email has not been verified" $ do
            let fixture = emptyFixture
                    {
                        _findUserByAuth = \_ -> return $ Just (1, False)
                    }
            runApp fixture (login auth) `shouldReturn` Left LoginErrEmailNotVerified

        it "should return a session if the login is successful" $ do
            let fixture = emptyFixture
                    {
                        _findUserByAuth = \_ -> return $ Just (1, True)
                        , _newSession = \uId -> if uId == 1 then return "sId" else unimplemented
                    }
            runApp fixture (login auth) `shouldReturn` Right "sId"

    describe "resolveSessionId" $ do
        it "should return Nothing if the session is not found" $ do
            let fixture = emptyFixture
                    {
                        _findUserIdBySessionId = \_ -> return Nothing
                    }
            runApp fixture (resolveSessionId "sId") `shouldReturn` Nothing

        it "should return UserId if the session is found" $ do
            let fixture = emptyFixture
                    {
                        _findUserIdBySessionId = \sId -> if sId == "sId" then return (Just 1) else unimplemented
                    }
            runApp fixture (resolveSessionId "sId") `shouldReturn` Just 1

    describe "getUser" $ do
        it "should return Nothing if the user is not found" $ do
            let fixture = emptyFixture
                    {
                        _findEmailFromUserId = \_ -> return Nothing
                    }
            runApp fixture (getUser 1) `shouldReturn` Nothing

        it "should return Email if the user is found" $ do
            let expected = Just (authEmail auth)
            let fixture = emptyFixture
                    {
                        _findEmailFromUserId = \uId -> if uId == 1 then return expected else unimplemented
                    }
            runApp fixture (getUser 1) `shouldReturn` expected