...
addAuth :: PG r m => D.Auth -> m (Either D.RegistrationErr (D.UserId, D.VerificationCode))
    ...
    case result of
        ...
        Left err@SqlError{sqlState = state, sqlErrorMsg = msg} ->
            if state == "23505" && "auths_email_key" `isInfixOf` msg
                then return $ Left D.RegistrationErrEmailTaken
...
setEmailAsVerified :: PG r m => D.VerificationCode -> m (Either D.EmailVerificationErr (D.UserId, D.Email))
setEmailAsVerified vCode = do
    ...
    case result of
        ...
        _ -> return $ Left D.EmailVerificationErrInvalidCode
    where
        qry = "update auths \
